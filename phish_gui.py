...
def report_generator(email_content, sender_subject, phishing_result, danger_result, lstm_result, transformer_results, url_results, email_rep_result, sender_email_address): 
    """
    Sends an email report from the sender to the recipient containing the specified subject and content.
    :param email_content: email body content
...
def fetch_email(server):
    """
    Fetches the latest email and processes it once.
    :param server: server connection (imap.gmail.com, PORT 993 over SSL)
    """
    ...
    if email_content:
        ...
        report_generator(email_content, subject, phishing_result, danger_result, LSTM_result, transformer_result, url_results, email_result, sender_email)
